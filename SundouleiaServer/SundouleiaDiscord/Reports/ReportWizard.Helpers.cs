...
public record ReportedProfileInfo(ReportedUserProfile Report, UserProfileData Profile, AccountClaimAuth ReportedAuth, AccountClaimAuth ReporterAuth);
public record ReportedChatInfo(ReportedRadar Report, UserProfileData Profile, AccountClaimAuth ReportedAuth, AccountClaimAuth ReporterAuth);

// Helpers class
public partial class ReportWizard
{
    /// <summary>
    ///     Grabs the database context from the Sundouleia VM host.
    /// </summary>
    private async Task<SundouleiaDbContext> GetDbContext()
        => await _dbContextFactory.CreateDbContextAsync().ConfigureAwait(false);

    private async Task<ReportedProfileInfo> GetProfileReport(SundouleiaDbContext db, int reportId)
    {
        var query = from report in db.ProfileReports.AsNoTracking()
                    where report.ReportID == reportId
                    join reportedProfile in db.UserProfileData
    ...
    private async Task<Dictionary<int, ReportedChatInfo>> GetChatReports(SundouleiaDbContext db)
    {
        var query = from report in db.RadarReports.AsNoTracking()
                    ...
                    join reportedAuth in db.Auth
                        on report.ReportedUserUID equals reportedAuth.UserUID
                    join reporterAuth in db.Auth
                        on report.ReporterUID equals reporterAuth.UserUID
    ...
    private async Task<bool> ValidateInteraction()
    {
        ...
        eb.WithDescription("This session has expired since you have either again pressed \"Start\" on the " +
            "initial message or the bot has been restarted.\n\n" +
