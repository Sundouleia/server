...
namespace SundouleiaDiscord.Commands;
#nullable enable
...
public class SundouleiaCommands : InteractionModuleBase
{
    ...
    [SlashCommand("unban", "Unbans a user from Sundouleia services.")]
        ...
        [Summary("timeFrame", "Use d for days, h for hours, or say now to clear all")] string timeFrame)
    {
        ...
        using IServiceScope scope = _services.CreateScope();
        SundouleiaDbContext? db = scope.ServiceProvider.GetService<SundouleiaDbContext>();
        // create a list of all users from the users table whose LastLogin time was greater than time current time - timespan
...
#pragma warning restore MA0011 // IFormatProvider is missing
    ...
    private async Task<EmbedBuilder> HandleUserInfo(EmbedBuilder eb, ulong id, string? secondaryUserUid = null, ulong? optionalUser = null, string? uid = null)
    {
        ...
        await using SundouleiaDbContext db = scope.ServiceProvider.GetRequiredService<SundouleiaDbContext>();
        ...
        if (primaryUser is null)
        {
            ...
            eb.WithDescription("No Sundouleia account was found associated to your Discord user");
            return eb;
        }
        ...
        else if ((optionalUser != null || uid != null))
        {
            ...
            {
                eb.WithTitle("No account");
                eb.WithDescription("The Discord user has no valid Sundouleia account");
                return eb;
            }
