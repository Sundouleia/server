...
public partial class AccountWizard
{
    ...
    private async Task HandleProfiles(EmbedBuilder eb, SundouleiaDbContext db, string uid)
    {
        ...
        var identity = await _multiplexer.GetDatabase().StringGetAsync("UID:" + dbUser.UID).ConfigureAwait(false);
        ...
        if (dbUser.Tier != CkVanityTier.NoRole)
        {
