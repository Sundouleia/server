    ...
    public async Task ComponentRelinkVerifyCheck(string verificationCode, string uid)
    {
        ...
        else
        {
            if (verified)
            {
                ...
                using var db = await GetDbContext().ConfigureAwait(false);
                var res = await HandleRelinkUser(db, uid).ConfigureAwait(false);
                ...
                eb.WithDescription("This is your private secret key. Do not share this private secret key with anyone. **If you lose it, it is irrevocably lost.**"
                                             ...
                                             + $"**{res}**"
                                             + Environment.NewLine + Environment.NewLine
                                             + "Enter this key in Sundouleia and hit save to connect to the service."
                                             + Environment.NewLine
                                             + "Have fun.");
                AddHome(cb);
            }
...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
    ...
    private async Task<(string, string)> HandleRelinkUser(SundouleiaDbContext db, string uid)
    {
